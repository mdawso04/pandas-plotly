...
| --- | --- | ---
| DF_COL_ADD_FIXED | Add a new column with a 'fixed' value as content | ```.DF_COL_ADD_FIXED('Tokyo', 'Location')```  ```.DF_COL_ADD_FIXED('Tokyo')```  
| DF_COL_ADD_INDEX |  Add a new column with a index/serial number as content | ```.DF_COL_ADD_INDEX()```  ```.DF_COL_ADD_INDEX('Serial number')```  ```.DF_COL_ADD_INDEX('Serial number', 0)```
| DF_COL_ADD_INDEX_FROM_0 |  Convenience method for DF_COL_ADD_INDEX | ```.DF_COL_ADD_INDEX_FROM_0()```  ```.DF_COL_ADD_INDEX_FROM_0('Serial number')```
| DF_COL_ADD_INDEX_FROM_1 |  Convenience method for DF_COL_ADD_INDEX | ```.DF_COL_ADD_INDEX_FROM_1()```  ```.DF_COL_ADD_INDEX_FROM_1('Serial number')```
